...
while (!found) {
	...
	let nonce = 0x01; //The nonce must be a hex literal! // 交易计数（必须是十六进制字面量）- 第二个合约（代理合约）使用 nonce=0x01
	...
	let contract_address = contract_address_long.substring(24); //Trim the first 24 characters. // 截取后 40 位得到部署合约地址（第二个合约/代理合约地址）

	if (pattern.test(contract_address)) {
		// 测试地址是否匹配模式
		const privateKey = sender.getPrivateKeyString(); // 获取私钥字符串
		console.log(`contract_address (proxy): ${contract_address}`); // 输出代理合约地址
		console.log(`sender address: ${address}`); // 输出发送者地址
		console.log(`privateKey: ${privateKey}`); // 输出对应私钥（请妥善保管）
		console.log(`注意：这是第二个合约（代理合约）地址，需要先部署第一个合约（nonce=0x00）`); // 提示信息
